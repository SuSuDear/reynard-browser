    ...
    static func configuration(
        ...
        return UIContextMenuConfiguration(identifier: UUID().uuidString as NSString, previewProvider: previewProvider) { _ in
            UIMenu(title: "", children: [
                UIAction(title: L10n.string("context_menu.share_image"), image: UIImage(named: "reynard.square.and.arrow.up")) { _ in
                    loadImage(from: url) { image in
                        presentShareSheet(image: image, from: presentingController, sourceView: sourceView)
                    }
                },
                UIAction(title: L10n.string("context_menu.save_to_photos"), image: UIImage(named: "reynard.square.and.arrow.down")) { _ in
                    loadImage(from: url) { image in
                        UIImageWriteToSavedPhotosAlbum(image, nil, nil, nil)
                    }
                },
                UIAction(title: L10n.string("common.copy"), image: UIImage(named: "reynard.document.on.document")) { _ in
                    loadImage(from: url) { image in
