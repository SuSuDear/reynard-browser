...
final class BookmarksViewController: UIViewController, UITableViewDataSource, UITableViewDelegate, UISearchBarDelegate, UIGestureRecognizerDelegate {
    ...
    private lazy var newFolderButton = UIBarButtonItem(
        title: L10n.string("bookmarks.new_folder"),
        style: .plain,
        target: self,
    ...
    private lazy var searchBar: UISearchBar = {
        ...
        searchBar.placeholder = L10n.string("bookmarks.search_placeholder")
    ...
    private let emptyLabel: UILabel = {
        let label = UILabel()
        label.text = L10n.string("bookmarks.no_matches")
    ...
    init(folderID: String? = nil, store: BookmarkStore = .shared) {
        ...
        title = L10n.string("bookmarks.title")
    }
    ...
    func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let editAction = UIContextualAction(style: .normal, title: L10n.string("common.edit")) { [weak self] _, _, completion in
            guard let self else {
                completion(false)
                return
            }
    ...
    fileprivate func makeBookmarkMenu() -> UIMenu {
        UIMenu(title: "", children: [
            makeSortMenu(),
            UIAction(
                title: L10n.string("bookmarks.show_folders_on_top"),
            ...
            UIMenu(title: "", image: nil, identifier: nil, options: .displayInline, children: [
                UIAction(title: L10n.string("bookmarks.edit_bookmarks"), image: UIImage(named: "reynard.pencil")) { [weak self] _ in
                    self?.setEditing(true, animated: true)
                },
                UIAction(title: L10n.string("bookmarks.new_folder"), image: UIImage(named: "reynard.folder.badge.plus")) { [weak self] _ in
    ...
    private func makeSortMenu() -> UIMenu {
        ...
        let sortOptions: [(title: String, order: BookmarkSortOrder)] = [
            (L10n.string("bookmarks.sort_none"), .none),
            (L10n.string("bookmarks.sort_date_added"), .date_added),
            (L10n.string("bookmarks.sort_name"), .name),
            (L10n.string("bookmarks.sort_address"), .address),
        ]
        let menu = UIMenu(
            title: L10n.string("bookmarks.sort_by"),
    ...
    private func makeBookmarkSections(from newItems: [BookmarkContentSnapshot]) -> [(title: String, items: [BookmarkContentSnapshot])] {
        guard Prefs.BookmarkSettings.placeFoldersOnTop else {
            ...
            return sortedItems.isEmpty ? [] : [(L10n.string("bookmarks.title"), sortedItems)]
        }
        ...
        return [
            (L10n.string("bookmarks.folders"), folders),
            (L10n.string("bookmarks.title"), bookmarks),
