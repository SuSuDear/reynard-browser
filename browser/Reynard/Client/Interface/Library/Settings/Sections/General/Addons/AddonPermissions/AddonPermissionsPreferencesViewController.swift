...
final class AddonPermissionsPreferencesViewController: SettingsTableViewController {
    ...
    private var permissionSections: [SectionModel] {
        ...
        if !requiredPermissions.isEmpty {
            sections.append(
                SectionModel(
                    headerTitle: L10n.string("addons.permissions.required"),
        ...
        if !optionalRows.isEmpty {
            sections.append(SectionModel(headerTitle: L10n.string("addons.permissions.optional"), displayedRows: optionalRows))
        }
        ...
        if let requiredDataCollectionDescription = AddonPermissionSupport.requiredDataCollectionDescription(for: metaData.requiredDataCollectionPermissions) {
            sections.append(
                SectionModel(
                    headerTitle: L10n.string("addons.data.required"),
        ...
        if !optionalDataCollectionPermissions.isEmpty {
            sections.append(
                SectionModel(
                    headerTitle: L10n.string("addons.data.optional"),
    ...
    init(addonID: String) {
        ...
        title = L10n.string("permissions.title")
    }
        ...
        Task { [weak self] in
            ...
            do {
                ...
                await MainActor.run {
                    ...
                    self.addon = addon
                    self.tableView.reloadData()
                    AlertPresenter.show(title: L10n.string("addons.error.update_permissions"), message: "\(error)")
                }
            }
        }
    }
