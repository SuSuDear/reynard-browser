...
final class AppearancePreferencesViewController: SettingsTableViewController {
    private enum Section: CaseIterable {
        ...
        var text: SettingsSectionText {
            ...
            case .addressBar:
                return SettingsSectionText(headerTitle: L10n.string("settings.appearance.address_bar"))
            case .tabs:
                return SettingsSectionText(headerTitle: L10n.string("settings.appearance.tabs"))
            case .pageZoom:
                return SettingsSectionText(headerTitle: L10n.string("settings.appearance.page_zoom"))
            }
        }
    ...
    private var displayedSections: [Section] {
        return Section.allCases.filter { !$0.rows.isEmpty }
    }

    init() {
        super.init(style: .insetGrouped)
        title = L10n.string("settings.general.appearance")
    }
    ...
    override func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        guard displayedSections.indices.contains(section) else { return 0 }
        return displayedSections[section].rows.count
    }

    override func sectionText(for section: Int) -> SettingsSectionText {
        guard displayedSections.indices.contains(section) else { return SettingsSectionText() }
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case .showFullWebsiteAddress:
            ...
            cell.textLabel?.text = L10n.string("settings.appearance.show_full_website_address")
            cell.selectionStyle = .none
            cell.accessoryView = showFullWebsiteAddressSwitch
            return cell
        case .landscapeTabBar:
            let cell = UITableViewCell(style: .default, reuseIdentifier: nil)
            cell.textLabel?.text = L10n.string("settings.appearance.landscape_tab_bar")
            cell.selectionStyle = .none
            cell.accessoryView = landscapeTabBarSwitch
            return cell
        case .pageZoom:
            let cell = UITableViewCell(style: .default, reuseIdentifier: nil)
            cell.textLabel?.text = L10n.string("settings.appearance.zoom_settings")
