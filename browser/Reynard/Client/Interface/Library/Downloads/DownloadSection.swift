...
extension DownloadSection {
    ...
    private static func appendRelativeSections(
        ...
        if !todayItems.isEmpty {
            sections.append(DownloadSection(title: L10n.string("date_section.today"), items: todayItems))
        }
        if !yesterdayItems.isEmpty {
            sections.append(DownloadSection(title: L10n.string("date_section.yesterday"), items: yesterdayItems))
        }
        if !previousSevenDayItems.isEmpty {
            sections.append(DownloadSection(title: L10n.string("date_section.previous_7_days"), items: previousSevenDayItems))
        }
        if !previousThirtyDayItems.isEmpty {
            sections.append(DownloadSection(title: L10n.string("date_section.previous_30_days"), items: previousThirtyDayItems))
        }
    }
