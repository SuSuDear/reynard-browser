...
final class TabOverviewPresentation {
    ...
    func cardSize(in collectionView: UICollectionView) -> CGSize {
        ...
        let tabCount = tabs(for: tabOverview.mode.tabMode).count
        let targetWidth = context.browserLayout.chromeMode == .phone
        ? (tabCount == 1 ? availableWidth * UX.phoneSingleCardWidthRatio : UX.phoneCardTargetWidth)
        ...
        let columns = tabCount == 1 ? 1 : max(UX.minimumTabCardColumnCount, computedColumns)
